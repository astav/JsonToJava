...
public class JsonToJava {
    ...
    public static void main(String[] args) throws IOException, ClassNotFoundException {
        ...
        String importBaseDir = args.length > 5 ? args[5] : null;
        List<String> importPackageNames = args.length > 6 ? Lists.newArrayList(args).subList(6, args.length) : null;
        @SuppressWarnings("unchecked") Map<String, Object> map = mapper.readValue(FileUtils.readFileToString(new File(args[0])), Map.class);
        Generator generator = new Generator(args[1], args[2], args[4], importBaseDir, importPackageNames);
